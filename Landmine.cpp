...
class LandmineOperator : public osgParticle::Operator {
public:
	LandmineOperator()
		: osgParticle::Operator(), _center(0, 0, 0), _axis(0, 0, 1), _wind(0, 0, 0) {}

	LandmineOperator(const LandmineOperator& copy, const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY)
		: osgParticle::Operator(copy, copyop), _center(copy._center), _axis(copy._axis), _wind(copy._wind) {}

	META_Object(osgParticle, LandmineOperator);
	...
	void operate(osgParticle::Particle* P, double dt)
	{
		...
		if (P->getAge() > 0.3)
		{
			P->setVelocity(-osg::Vec3(0.1, 0.1, 0.7));
		}
		//else
		//{
...
osg::Node* createLandmine(const osg::Vec3& position, float scale)
{
	...
	placer->setRadiusRange(0.0f, 0.1f * scale);
	...
	osg::ref_ptr<LandmineOperator> motion = new LandmineOperator;
